...
def client_main(args):
    ...
    rpc = RpcClient(args)
    rpc.server_name = server_name
    response = rpc.connect(**{'user': "test_user"})
    ...
    # create game session
    response = rpc.create_session(**{'user': 'test_user', 'sname': 'test_session', 'player_count': 4})
    print "Create game: " + str(response)

    # new connection, join to session
    rpc.connect(**{'user': "test_user2"})
    response = rpc.join_session(**{'user': "test_user2", 'sname': 'test_session'})
    print "Join session: " + str(response)

    #send ship placement
    response = rpc.send_ship_placement(**{'user': "test_user2", 'sname': 'test_session',
    ...
    response = rpc.ready(**{'user': "test_user2", 'sname': 'test_session'})
    print response
    #response = rpc.call('test_rpc_ready', **{'user': "test_user2", 'sname': 'test_session'})
    #print response

    response = rpc.start_game(**{'user': "test_user", 'sname': 'test_session'})
    print response

    # leave from session
    response = rpc.leave_session(**{'user': "test_user", 'sname': 'test_session'})
    response = rpc.leave_session(**{'user': "test_user2", 'sname': 'test_session'})
    print "Leave from session" + str(response)

    # leave from server
    response = rpc.disconnect(**{'user': "test_user"})
    response = rpc.disconnect(**{'user': "test_user2"})
